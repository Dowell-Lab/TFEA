...
def inputs_processor():
    ...
    """
    description_text = (
        "Merges region calls (mu) generated by Tfit, or other "
        "peak calling functions across multiple samples and replicates."
    )

    input_help = (
        "Input file (full path) containing bedfiles, sample ID's "
    ...
    input_details = (
        "Header line indicated by '#' character must be included "
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Output file basename (full path, sans "
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Sorted bedfile (full path) containing the "
    ...
    if not args.output:
        raise TypeError(
            "Please specify output filename with '-o' flag. "
    ...
    if args.input:
        with open(args.input, "r") as f:
            ...
            for i, col in enumerate(header):
                if col == "file":
                    ...
                    raise ValueError(
                        "Header only contains 'file', 'sampid' "
        ...
        raise TypeError(
            "Please specify input file with '-i' flag. "
...
def log_initializer(tfit_filenames, groupings, miscallfile, logfile):
    ...
    miscallfile.write(
        "# This file contains regions which were identified not "
...
def tfit_dict_initializer(
    interest_regions, chromesome_flag=True, bed_region_flag=True
):
    """
...
def tfit_file_reader(filename, sampid, tfit_dict):
    ...
    with open(filename, "r") as f:
        ...
        try:
            region_key = next(
                key
                for key in tfit_dict[chromesome].keys()
...
#            3) ...
def mu_dict_generator(
    tfit_filenames, interest_regions, sampids=["sampid_NA"], verbose=False
):
    """
    ...
    assert isinstance(
        tfit_filenames, (list, tuple)
    ), "'tfit_filenames' must be of type 'list'"
    assert isinstance(sampids, (list, tuple)), "'sampid' must be of type 'list'"
    ...
    # If sample ID's are provided but not right length, print warning
        ...
        if verbose:
            print(
                (
                    "WARNING: Length of 'sampids' does not match "
...
def prob_list_generator(xvals, params=None, dist="normal"):
    ...
    if dist == "normal":
        ...
        mu_sig = (
            params[1] - params[0]
        ) / 4  ## THIS 4 IS A KEY FACTOR IN INTERPRETTING TFIT INTERVALS!!
...
def prob_list_formatter(region, mu_list, dist="normal"):
    ...
    try:
        ...
        print(
            (
                "'mu_list' is not of the right format -- list of tuples"
                "(start, stop, cov, 'sampID')"
            )
        )
...
def mu_sig_extract(mu_list):
    ...
    mu_sig_list = [
        (
            round((i[1] + i[0]) / 2),
            round((i[1] - i[0]) / 4),
        )  # THIS FACTOR SAME AS ONE IN prob_list_generator()!!!
...
def collision_resolver(mu_sig_list):
    ...
    for i, mu in enumerate(mus[:-1]):
        ...
        if overlap_check(
            (pos1 - sig1, pos1 + sig1), (pos2 - sig2, pos2 + sig2)
        ):
...
## Load merged bedfile
merge_regions = bedfile_reader(
    union_bedfile, bedGraph=False, print_header=False, count=False
)
if verbose:
...
tfit_dict = mu_dict_generator(
    list(tfit_filenames), merge_regions, sampids=list(sampids), verbose=verbose
)
...
with open(outbedfile, "w") as output:
    ...
    for chromosome in tfit_dict.keys():
        for region in tfit_dict[chromosome].keys():
            ...
            if verbose:
                sys.stdout.write(
                    "\rProcessed {} of {} regions".format(count, total)
                )
                count += 1
            ...
            sample_prob_dict = prob_list_formatter(
                region, mu_list, dist="normal"
            )  # CHECK!!!

            # Calculate combined probability array (function of base position),
            # from 'groups' and the probability lists in sample_prob_dict()
            comb_prob = combined_prob_calculator(
                sample_prob_dict, groups=groupings
            )  # FIX!!!
