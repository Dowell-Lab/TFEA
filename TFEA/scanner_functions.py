...
def fimo_parse(fimo_file=None, largewindow=None, retain='score'):
    ...
    with open(fimo_file) as F:
        ...
        if len(header) > 1:
            ...
            for line in F:
                line = line.strip('\n').split('\t')
                fc, pval, rank = line[name_index].split(',')
                rank = int(rank)
                if max_rank == None:
                    max_rank = rank
                if min_rank == None:
                ...
                if rank not in d:
                    d[rank] = [rank, fc, pval, score, distance]
                elif retain == 'score':
...
def bedtools_closest(motif, genomehits=None, ranked_center_file=None, 
    ...
    '''
    try:
        motif_path = os.path.join(genomehits, motif + '.bed')
        bedtools_closest_out = os.path.join(tempdir, motif + '.closestBed.bed')

        command = ("bedtools sort " + ranked_center_file + " | bedtools closest "
                    "-D ref -t first -a stdin -b " + motif_path + " > "
