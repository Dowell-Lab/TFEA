...
def run(ranked_center_file,MOTIF_PATH):
    ...
    os.system("bedtools closest -d -t first -a " + ranked_center_file.split('.bed')[0] + ".sorted.bed -b " + MOTIF_PATH + " > " + '/' + '/'.join(ranked_center_file.split('/')[:-1]) + '/' + MOTIF_PATH.split('/')[-1] + ".sorted.distance.bed")
